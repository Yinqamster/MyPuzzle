...
class Puzzle2ViewController: UIViewController {
    ...
    */
    ...
    func setUpImg(){
        ...
        puzzle.pointInfo.removeAll(keepingCapacity: true)
        
        puzzle.kImgWidth = (UIScreen.main.bounds.size.width - 60) / CGFloat(puzzle.kCol)
        puzzle.screenwidth=UIScreen.main.bounds.size.width;
        puzzle.screenheight=UIScreen.main.bounds.size.height;
        
        for i in 0..<(puzzle.kCol * puzzle.kCol) {
            let col = i % puzzle.kCol
            let row = i / puzzle.kCol
            let x = 30 + CGFloat(col) * puzzle.kImgWidth
            let y = 180 + CGFloat(row) * puzzle.kImgWidth
            ...
            let P : CGPoint = CGPoint(x:imgView.frame.minX,y:imgView.frame.minY)
            puzzle.pointInfo.append(P)
         //   print(P.x,P.y)//////////
...
extension Puzzle2ViewController: FZImagePickerDelegate{
